...
async function loadStats(){try{const s=await api('/api/stats');document.getElementById('stat-total').textContent=s.total||0;document.getElementById('stat-transit').textContent=(s.in_transit||0)+(s.shipped||0);document.getElementById('stat-delivered').textContent=s.delivered||0;document.getElementById('count-all').textContent=s.total||0;document.getElementById('count-pending').textContent=s.pending||0;document.getElementById('count-shipped').textContent=s.shipped||0;document.getElementById('count-delivered').textContent=s.delivered||0;document.getElementById('count-priority').textContent=s.priority||0;document.getElementById('count-received').textContent=s.received||0;}catch(e){console.error(e);}}
...
async function deleteShipment(id){if(!confirm('Remove this shipment?'))return;try{await api('/api/shipments/'+id,{method:'DELETE'});allShipments=allShipments.filter(s=>s.id!==id);allGroups=groupShipments(allShipments);await loadAll();showToast('Removed');}catch(e){showToast('Failed',true);}}
