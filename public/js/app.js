...
function setFilter(f,btn){currentFilter=f;document.querySelectorAll('.filter-item[data-filter]').forEach(b=>b.classList.remove('active'));btn.classList.add('active');const titles={all:'All packages',transit:'In transit',shipped:'Shipped',delivered:'Delivered',received:'Received'};document.getElementById('page-title').textContent=titles[f]||'Packages';applyFilters();}
...
async function saveEdit(){if(!editingId)return;const tracking_number=document.getElementById('edit-tracking').value.trim();const po_number=document.getElementById('edit-po').value.trim();const order_number=document.getElementById('edit-order').value.trim();const description=document.getElementById('edit-desc').value.trim();const ship_to=document.getElementById('edit-shipto').value.trim();const project_id=document.getElementById('edit-project').value||null;try{const updated=await api('/api/shipments/'+editingId,{method:'PATCH',body:{tracking_number,po_number,order_number,description,ship_to}});if(project_id!==undefined)await api('/api/shipments/'+editingId+'/assign',{method:'POST',body:{project_id}});const idx=allShipments.findIndex(s=>s.id===editingId);if(idx>=0){allShipments[idx]={...allShipments[idx],...updated,_vendor:allShipments[idx]._vendor,project_id};}allGroups=groupShipments(allShipments);document.getElementById('edit-modal').style.display='none';editingId=null;await loadProjects();applyFilters();showToast('Shipment updated');}catch(e){showToast('Save failed',true);}}
