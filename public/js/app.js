...
function chipHTML(carrier,vendor){const c=(carrier||'').toLowerCase();const label={ups:'UPS',fedex:'FedEx',usps:'USPS',amazon:'AMZN',dhl:'DHL'}[c]||(carrier||'?');const cls='carrier-chip '+((['ups','fedex','usps','amazon','dhl'].includes(c))?c:'other');const logo=CARRIER_LOGOS[c]||null;if(logo)return '<div class="'+cls+'"><img src="'+logo+'" width="22" height="22" alt="'+esc(label)+'" onerror="this.style.display=\'none\';this.nextSibling.style.display=\'inline\'"><span style="display:none;font-size:9px;font-weight:700;">'+esc(label)+'</span></div>';return '<div class="'+cls+'" style="font-size:9px;font-weight:700;">'+esc(label)+'</div>';}
